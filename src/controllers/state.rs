...
impl Default for DivePlanState {
    fn default() -> Self {
        Self {
            dive_profiles: Default::default(),
            dive_steps: Default::default(),
...
mod state_should {
    ...
    #[test]
    fn get_last_dive_step_state() {
        let mut state = DivePlanState::default();
        state.dive_steps = vec![default_dive_step_test_fixture(), dive_step_test_fixture()];

        let dive_step = state.get_last_dive_step();

        assert_eq!(50, dive_step.depth);
        assert_eq!(10, dive_step.time);
    }
    ...
    fn get_last_cylinders_state() {
        ...
        assert_eq!(12, cylinders[0].cylinder_volume);
        assert_eq!(200, cylinders[0].cylinder_pressure);
        assert_eq!(2400, cylinders[0].initial_pressurised_cylinder_volume);
        assert_eq!(21, cylinders[0].gas_mixture.oxygen);
        assert_eq!(10, cylinders[0].gas_mixture.helium);
        assert_eq!(69, cylinders[0].gas_mixture.nitrogen);
        assert_eq!(12, cylinders[0].gas_management.surface_air_consumption_rate);
        assert_eq!(0, cylinders[0].gas_management.gas_used);
        assert_eq!(2400, cylinders[0].gas_management.gas_remaining);
    }

    fn default_dive_profile_test_fixture() -> DiveProfile {
        DiveProfile::default()
    }
    ...
    fn default_dive_step_test_fixture() -> DiveStep {
        DiveStep::default()
    }
    ...
    fn default_cylinder_test_fixture() -> Cylinder {
        Cylinder::default()
    }
