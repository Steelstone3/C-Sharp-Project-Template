...
mod dive_profile_should {
    ...
    fn read_from_the_file_and_parse_to_the_dive_profile_model() {
        let expected_dive_profiles = vec![dive_profile_test_fixture(), dive_profile_test_fixture()];

        upsert_dive_profile_file(&expected_dive_profiles)
            .expect("integration test dive_profile.json file didn't upsert");
        let dive_profiles = read_dive_profile_file();

        assert_eq!(expected_dive_profiles, dive_profiles);
    }
